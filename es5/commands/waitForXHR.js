...
const clientListen = function (clientUrlPattern, clientListenId) {
    ...
    if (!XMLHttpRequest.customized) {
        ...
        XMLHttpRequest.prototype.send = function (data) {
            if (this.listen) this.listen.requestData = data;else this.listen = { requestData: data };
...
const clientPoll = function (clientListenId) {
    if (window[clientListenId]) {
        // alert(window[clientListenId].status);
        const keys = Object.keys(window[clientListenId]);
        /*if (keys.length)
            alert(keys.join(','));*/
...
WaitForXHR.prototype.poll = function () {
    ...
    this.api.execute(clientPoll, [command.xhrListenId], function (result) {
        ...
        } else command.reschedulePolling.call(command);
    });
};
...
WaitForXHR.prototype.command = function (urlPattern = '', timeout, trigger = identity, callback = identity) {
    ...
    if (trigger) {
        if (typeof trigger === "function") trigger(this.client.api);else if (typeof trigger === "string") api.click(trigger);
    }
    // console.log('Done');
    return this;
};
